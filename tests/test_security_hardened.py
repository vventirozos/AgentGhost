...
def test_docker_sandbox_injects_proxy(mock_docker, mock_tor_proxy, mock_tor_proxy_h):
    ...
    # Switched from positional list to a callable so new install-gate
    # probes (the pre/post Chromium `find` calls) don't shift every
    # subsequent entry out of alignment. The contract the test cares
    # about is unchanged: marker absent → install fires → tor wiring
    # happens, and apt-get runs with an empty environment dict.
    state = {"post_install": False}

    def _exec_run(cmd, *args, **kwargs):
        if "test -f /root/.supercharged.v2" in cmd:
            return (1, b"")       # marker absent → trigger install
        if "find /root/.cache/ms-playwright" in cmd:
            # Pre-install: no binary; post-install: binary present.
            if state["post_install"]:
        ...
        if "playwright install" in cmd:
            ...
            return (0, b"")
        if "test -f /usr/bin/tor" in cmd:
            return (1, b"")       # tor binary absent → install tor
        if cmd.startswith("pgrep -x tor"):
            return (1, b"")       # tor daemon not running → start it
        return (0, b"")           # apt, sudoers, pip, touch, rm, tor start
