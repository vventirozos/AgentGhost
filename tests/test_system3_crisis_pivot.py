...
Covers:
  ...
  6. handle_chat interception: triggered at exactly execution_failure_count == 4.
...
# ---------------------------------------------------------------------------
# 6. handle_chat: interception triggers at count == 4 and pivots the plan
# ---------------------------------------------------------------------------
...
async def test_handle_chat_triggers_pivot_at_second_failure():
    ...
    # Turn 1-4: LLM requests execute → fail
    # After pivot: LLM answers directly
    llm_sequence = [exec_call, exec_call, exec_call, exec_call, final_answer]
    ...
    with patch.object(agent, "_run_system_3_pivot", new=AsyncMock(return_value=pivot_result)) as mock_pivot, \
         patch("ghost_agent.tools.file_system.tool_list_files", new=AsyncMock(return_value="empty")), \
...
async def test_handle_chat_pivot_resets_failure_count():
    ...
    # Four failures → pivot → final answer
    llm_sequence = [exec_call, exec_call, exec_call, exec_call, final_answer]
    ...
    assert "failed 6 times" not in result_content.lower(), \
...
async def test_handle_chat_pivot_injects_user_message():
    ...
    agent = _make_agent()
    agent.context.llm_client.chat_completion = AsyncMock(
        side_effect=[exec_call, exec_call, exec_call, exec_call, _llm_ok("Fixed!")]
    )
    agent.available_tools["execute"] = AsyncMock(return_value=EXEC_FAIL_RESULT)
...
async def test_handle_chat_pivot_not_triggered_on_first_failure():
    """
    The pivot should only fire at count == 4, not at count == 1.
    """
    exec_call = _execute_tool_call()
...
async def test_handle_chat_falls_to_hard_limit_when_pivot_empty():
    ...
    llm_sequence = [exec_call, exec_call, exec_call, exec_call, exec_call, exec_call, final_answer]
    ...
    async def mock_llm(payload, **kwargs):
        for m in payload.get("messages", []):
            ...
            if "failed too many times" in content_lower or "failed 6 times" in content_lower:
    ...
    mock_llm.call_count = 0
    agent.context.llm_client.chat_completion = mock_llm
    agent.context.args.max_context = 999999
    ...
    # The strike cap now bails out of the turn loop directly instead of
    # giving the model another turn to produce a "graceful" final, so the
    # alert may or may not be passed to the LLM. The invariant that matters
    # is that it was *recorded* in the persisted message history.
    persisted = body.get("messages", [])
    persisted_alert = [
        m for m in persisted
        if "failed too many times" in str(m.get("content", "")).lower()
        or "failed 6 times" in str(m.get("content", "")).lower()
    ]
    assert len(hard_limit_injected) > 0 or len(persisted_alert) > 0, \
        "Hard-limit alert was not recorded after pivot returned empty"
