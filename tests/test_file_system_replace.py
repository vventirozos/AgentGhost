...
async def test_tool_replace_file_no_match(sandbox_dir):
    ...
    # Error message was updated to also surface the nearest file content
    # so the model can self-correct instead of looping on stale old_text.
    assert "SYSTEM INSTRUCTION" in res
...
async def test_tool_replace_file_heuristic_multiple_matches(sandbox_dir):
    ...
    assert "SYSTEM INSTRUCTION: Multiple instances of this text block found" in res
...
async def test_tool_file_system_replace_missing_args(sandbox_dir):
    ...
    assert "SYSTEM INSTRUCTION: You used operation='replace' but forgot to specify 'replace_with'" in res1
...
@pytest.mark.asyncio
async def test_tool_replace_file_aider(sandbox_dir):
    test_file = sandbox_dir / "test.txt"
    test_file.write_text("def my_func():\n    print('hello')\n    return True")
