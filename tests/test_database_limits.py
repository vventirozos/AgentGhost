...
async def test_database_fetch_limit_enforced():
    ...
    with patch.dict(sys.modules, {
        ...
        # 2. Result should contain the mock table output and the new
        # truncation marker. Post-audit, the marker switched from a
        # generic "... [Truncated ...]" tag to an actionable hint
        # explaining max-display-limit semantics + how to dig deeper.
        assert "Mock Table Output" in result
        assert "max display limit" in result
