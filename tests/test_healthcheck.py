...
def mock_context():
    ...
    context.scheduler = None  # Removed in the apscheduler refactor.
    # Biological daemon: a live (not-done) asyncio.Task analogue.
    bio = MagicMock()
    bio.done.return_value = False
    bio.cancelled.return_value = False
    context.biological_task = bio
    context.memory_bus = MagicMock()
    return context
...
async def test_check_health_basic(mock_context):
    ...
    with patch("ghost_agent.tools.system.platform.system", return_value="Linux"), \
        ...
        assert "Agent Internals: LLM=Active, Memory=Active, Graph=Offline, Sandbox=Active, Bus=Active, Biological Daemon=Running" in result
...
@pytest.mark.asyncio
async def test_check_health_biological_daemon_offline(mock_context):
    """When no biological_task is wired, the report must say Offline (not Unknown)."""
    mock_context.biological_task = None
    with patch("ghost_agent.tools.system.psutil"), \
         patch("ghost_agent.tools.system.subprocess.run"), \
         patch("ghost_agent.tools.system.curl_requests", None), \
         patch("ghost_agent.tools.system.httpx.AsyncClient") as mock_client_cls:
        ...
        mock_client_cls.return_value.__aenter__.return_value = mock_client
        result = await tool_check_health(context=mock_context)
    assert "Biological Daemon=Offline" in result
    assert "Unknown" not in result


@pytest.mark.asyncio
async def test_check_health_biological_daemon_crashed(mock_context):
    """A finished task with an exception must surface as Crashed(<ExcType>)."""
    bio = MagicMock()
    bio.done.return_value = True
    bio.cancelled.return_value = False
    bio.exception.return_value = RuntimeError("daemon boom")
    mock_context.biological_task = bio
    ...
    with patch("ghost_agent.tools.system.psutil"), \
        ...
        mock_client_cls.return_value.__aenter__.return_value = mock_client
        result = await tool_check_health(context=mock_context)
    assert "Biological Daemon=Crashed (RuntimeError)" in result


@pytest.mark.asyncio
async def test_check_health_biological_daemon_cancelled(mock_context):
    bio = MagicMock()
    bio.done.return_value = True
    bio.cancelled.return_value = True
    mock_context.biological_task = bio
    ...
    with patch("ghost_agent.tools.system.psutil"), \
        ...
        mock_client_cls.return_value.__aenter__.return_value = mock_client
        result = await tool_check_health(context=mock_context)
    assert "Biological Daemon=Cancelled" in result


@pytest.mark.asyncio
async def test_check_health_memory_bus_offline(mock_context):
    mock_context.memory_bus = None
    with patch("ghost_agent.tools.system.psutil"), \
         patch("ghost_agent.tools.system.subprocess.run"), \
         patch("ghost_agent.tools.system.curl_requests", None), \
         patch("ghost_agent.tools.system.httpx.AsyncClient") as mock_client_cls:
