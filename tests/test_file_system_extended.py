...
async def test_file_search(temp_dirs):
    ...
    sandbox_manager.execute = MagicMock(return_value=("code.py:1:def main(): pass\n", 0))
    
    # Search for code
    res = await tool_file_search(pattern="def main", sandbox_dir=sandbox, sandbox_manager=sandbox_manager)
    assert "code.py" in res
    
    # Search for notes
    sandbox_manager.execute.return_value = ("notes.txt:1:TODO: fix bug\n", 0)
    res2 = await tool_file_search(pattern="TODO", sandbox_dir=sandbox, sandbox_manager=sandbox_manager)
