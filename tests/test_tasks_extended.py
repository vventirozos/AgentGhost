...
@pytest.mark.asyncio
async def test_list_tasks_filters_system_job(mock_scheduler):
    job1 = MagicMock()
    job1.id = "job_1"
    job1.name = "Test"
    job1.next_run_time = "today"
    job2 = MagicMock()
    job2.id = "idle_dream_monitor"
    job2.name = "System monitor"
    mock_scheduler.get_jobs.return_value = [job1, job2]
    res = await tool_list_tasks(mock_scheduler)
    assert "job_1" in res
    assert "System monitor" not in res
    assert "idle_dream_monitor" not in res
