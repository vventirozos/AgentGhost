...
def test_docker_timeout_kill_flag():
    ...
    def exec_side_effect(*args, **kwargs):
        ...
        if args and "test -f" in args[0]:
            return (0, b"")
        if args and "find /root/.cache/ms-playwright" in args[0]:
            # Chromium binary probe — return a hit so the steady-state
            # gate skips provisioning; this test only exercises execute().
