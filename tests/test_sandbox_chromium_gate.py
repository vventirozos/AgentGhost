...
def test_v5_marker_gate_name_pinned():
    """The marker path is part of the install contract with
    sandbox/Dockerfile. Pin it here so renaming in one place without
    the other is a loud test failure. (v5 = 2026-07-12, adds stockfish;
    v4 added iproute2/flask/python-chess.)"""
    import inspect
    ...
    assert "/root/.supercharged.v10" in src, (
        "marker path drifted from /root/.supercharged.v10 — update "
        "sandbox/Dockerfile in lockstep"
    )


def test_provisioning_bakes_ss_flask_chess_and_stays_in_sync():
    """ss (iproute2) + flask + python-chess are pre-baked (2026-07-12) so
    "host a web app / chess service" requests stop re-installing them
    mid-task (~24s serial thrash observed live). The runtime installer in
    docker.py and the build-time sandbox/Dockerfile MUST agree — a package
    added to one but not the other means a from-scratch build differs from a
    runtime provision. Pin both here."""
    import inspect
    from pathlib import Path
    runtime = inspect.getsource(DockerSandbox._ensure_running_impl)
    dockerfile = (Path(__file__).resolve().parents[1] / "sandbox"
                  / "Dockerfile").read_text()
    for needle in ("iproute2", "flask", "python-chess", "stockfish"):
        assert needle in runtime, f"{needle} missing from docker.py provisioner"
        assert needle in dockerfile, f"{needle} missing from sandbox/Dockerfile"
    # And the marker both write must be the SAME version.
    assert "/root/.supercharged.v10" in runtime
    assert ".supercharged.v10" in dockerfile


def test_legacy_v1_marker_treated_as_unprovisioned():
    """An image from the pre-v2 era has /root/.supercharged but not
    /root/.supercharged.v10. The gate must detect this and trigger a
    ...
    v2_absent = _exec_run_map({
        "test -f /root/.supercharged.v10": (1, b""),
    })
    ...
    code, _ = sb.container.exec_run("test -f /root/.supercharged.v10")
    assert code != 0
...
def test_marker_present_but_binary_missing_triggers_reinstall():
    ...
    responses = _exec_run_map({
        "test -f /root/.supercharged.v10": (0, b""),     # marker: ok
        "find /root/.cache/ms-playwright": (0, b""),    # no binary path
    })
    sb.container.exec_run = MagicMock(side_effect=responses)
    marker_ok = (sb.container.exec_run("test -f /root/.supercharged.v10")[0] == 0)
...
def test_both_marker_and_binary_present_skips_install():
    sb = _make_sandbox_stub()
    responses = _exec_run_map({
        "test -f /root/.supercharged.v10": (0, b""),
...
def test_post_install_verification_refuses_to_mark_on_missing_binary():
    ...
    /root/.supercharged.v10. A hypothetical install that exited 0 but
...
def test_dockerfile_sets_v3_marker():
    ...
    from pathlib import Path
    dockerfile = Path(__file__).resolve().parent.parent / "sandbox" / "Dockerfile"
    assert "/root/.supercharged.v10" in dockerfile.read_text(), (
        "sandbox/Dockerfile must touch /root/.supercharged.v10 — the "
