...
async def test_tool_execute_stateful_wrapper():
    ...
    with patch("ghost_agent.tools.execute._get_safe_path") as mock_safe_path, \
        ...
        def safe_path_side_effect(sandbox_dir, filename):
            if str(filename) == ".jupyter_runner.py":
        ...
        # Verify the wrapper file gets the Jupyter client wrapper
        wrapper_written = mock_path_wrapper.written_content
        assert "import sys, json, re, time" in wrapper_written
        assert "from jupyter_client import BlockingKernelClient" in wrapper_written
        assert "kc.execute(code)" in wrapper_written
        assert "msg = kc.get_iopub_msg(timeout=1)" in wrapper_written
        assert "sys.stdout.write" in wrapper_written
