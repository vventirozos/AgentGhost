...
_FALLBACK_HINTS = {
    "execute": [
        ...
        ("command not found", "The sandbox container is minimal — that utility isn't installed. Check availability first with `command -v <tool>`, then use a built-in equivalent: a byte peek → `od -A x -t x1z <f>` (or python3 with open(...,'rb')); text processing → python3."),
    ],
    "file_system": [
