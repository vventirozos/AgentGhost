...
TOOL_DEFINITIONS = [
    ...
    {"type": "function", "function": {"name": "manage_tasks", "description": "Consolidated task manager (create, list, stop, stop_all). Tasks persist across agent restarts. A task re-created with the SAME name replaces the previous schedule.", "parameters": {"type": "object", "properties": {"action": {"type": "string", "enum": ["create", "list", "stop", "stop_all"]}, "task_name": {"type": "string", "description": "A short name for the task (required for 'create')."}, "cron_expression": {"type": "string", "description": "Standard cron format OR 'interval:seconds' (e.g., 'interval:60' for every minute). Required for 'create'. NOTE: cron times are interpreted in UTC, not local time — for a local-time schedule, convert first (e.g. 09:00 in Athens summer = '0 6 * * *')."}, "prompt": {"type": "string", "description": "The instruction the background agent should execute (required for 'create')."}, "task_identifier": {"type": "string", "description": "The ID of the task to kill (required for 'stop')."}}, "required": ["action"]}}},
